...
export default function GolfScoringApp() {
  ...
  const finishMatch = async () => {
    ...
    // Update player stats (MVP tracking)
    ...
    // pointsContributed: for each matchup pair, if a player's pairing won, credit them their share of pts
    // For perHole (Modified Scramble): credit each player on a winning pairing their share of hole wins
    ...
    if (fmt.perHole) {
      // Modified Scramble: for each hole won, credit the players of the winning pairing
      // A pairing "contributed" if their net score was the team's best that hole
      for (let h=1;h<=m.holes;h++) {
        const res = calcHoleResults(m,h,localScores,tee);
        const w = res?.matchupResults[0]?.winner;
        if (!w || w==='tie') continue;
        const winTeamIds = w==='t1p' ? t1Ids : t2Ids;
        // Find which pairing had the best net (the contributing pair)
      ...
      // Standard formats: credit players whose pairing won their matchup
      const pairs = getMatchupPairs(m.format);
      for (const [a,b] of pairs) {
        let at1=0,at2=0;
        for (let h=1;h<=m.holes;h++) {
    ...
    // netUnderPar: count holes where a player's pairing's net score beat par
    for (let h=1;h<=m.holes;h++) {
      ...
      for (const pk of Object.keys(m.pairings)) {
        ...
        if (net < hd.par) {
          ids.forEach(id => { playerNetUnderPar[id]=(playerNetUnderPar[id]||0)+1; });
        }
      }
    }
              ...
              <div key={p.id} className={`p-3 rounded-xl border ${i===0?'border-yellow-400 bg-yellow-50':'border-gray-200 bg-gray-50'}`}>
                <div className="flex items-center justify-between flex-wrap gap-3">
                  ...
                  <div className="flex gap-4 text-center text-xs">
                    {([
                      ['Pts',    p.pts.toFixed(1),           'text-green-600'],
                      ['Net-Par',p.net,                      'text-purple-600'],
                      ['Skins',  p.skins.toFixed(1),         'text-orange-600'],
                      ['Matches',`${p.stats?.matchesWon||0}/${p.stats?.matchesPlayed||0}`,'text-blue-600'],
                    ] as const).map(([l,v,c])=>(
